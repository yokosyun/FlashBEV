...
__global__ void flash_bevpool_grad_kernel(
  ...
  int out_offset = batch_idx * grid_x * grid_y * num_channels + 
  ...
  float inv_valid_count = (valid_count > 0) ? 1.0f / valid_count : 0.0f;
  ...
  for (int cam_idx = 0; cam_idx < num_cameras; cam_idx++) {
    ...
    for (int z_idx = 0; z_idx < grid_z; z_idx++) {
      ...
      float weight = depth_weight * grad_out * inv_valid_count;
