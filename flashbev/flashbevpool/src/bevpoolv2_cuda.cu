...
*/
__global__ void bev_pool_v2_classification_nearest_kernel(int c, int n_intervals,
  ...
  for(int i = 0; i < interval_length; i++){
    ...
    accumulator += *cur_feat * *cur_depth;
  }
...
/*
  ...
  Backward equations:
    Forward: out[rank_bev, c] = Σ_i (feat[rank_feat[i], c] * depth[i])
    ∂L/∂depth[i] = Σ_c (grad_out[rank_bev, c] * feat[rank_feat[i], c])
    ∂L/∂feat[rank_feat[i], c] = grad_out[rank_bev, c] * depth[i]
*/
__global__ void bev_pool_v2_classification_nearest_grad_kernel(int c, int n_intervals,
  ...
  float* __restrict__ feat_grad)
  {
    ...
    for(int cur_c = 0; cur_c < c; cur_c++)
    {
      for(int i = 0; i < interval_length; i++)
      {
        ...
        cur_depth = depth + ranks_depth[interval_start+i];
        cur_feat_grad = feat_grad + ranks_feat[interval_start+i] * c + cur_c;
        atomicAdd(cur_feat_grad, *cur_out_grad * *cur_depth);
      }
    }
}

__global__ void bev_pool_v2_regression_nearest_kernel(int c, int n_intervals,
                                  const float *__restrict__ depth,
                                  const float *__restrict__ feat,
                                  const int *__restrict__ ranks_feat,
                                  const int *__restrict__ ranks_bev,
  ...
  for(int i = 0; i < interval_length; i++){
    cur_feat = feat + ranks_feat[interval_start+i] * c + cur_c;
    cur_depth = depth + interval_start+i;
    accumulator += (*cur_feat) * (*cur_depth);
  }
...
__global__ void bev_pool_v2_regression_nearest_grad_kernel(int c, int n_intervals,
  const float *__restrict__ out_grad,
  const float *__restrict__ depth,
  const float *__restrict__ feat,
  const int *__restrict__ ranks_feat,
...
void bev_pool_v2_classification_nearest(int c, int n_intervals, const float* depth, const float* feat, const int* ranks_depth,
  const int* ranks_feat, const int* ranks_bev, const int* interval_starts, const int* interval_lengths, float* out) {
  bev_pool_v2_classification_nearest_kernel<<<(int)ceil(((double)n_intervals * c / 256)), 256>>>(
...
void bev_pool_v2_classification_nearest_grad(int c, int n_intervals, const float* out_grad,
  ...
  bev_pool_v2_classification_nearest_grad_kernel<<<(int)ceil(((double)n_intervals / 256)), 256>>>(
...
void bev_pool_v2_regression_nearest(int c, int n_intervals, const float* depth, const float* feat,
  const int* ranks_feat, const int* ranks_bev, const int* interval_starts, const int* interval_lengths, float* out) {
  bev_pool_v2_regression_nearest_kernel<<<(int)ceil(((double)n_intervals * c / 256)), 256>>>(
...
void bev_pool_v2_regression_nearest_grad(int c, int n_intervals, const float* out_grad,
  ...
  const int* ranks_bev, const int* interval_starts, const int* interval_lengths, float* depth_grad, float* feat_grad) {
  bev_pool_v2_regression_nearest_grad_kernel<<<(int)ceil(((double)n_intervals / 256)), 256>>>(
