...
class SamplingVT(BaseModule):
    ...
    def sampling_vt(self, input, features_pv, depths):
        ...
        if self.fuse_projection:
            ...
            if self.use_bev_pool:
                ...
                Z_DIM = -3
                CAM_DIM = 1
                bev_feat = features_3d.unflatten(0, (B, N)).sum(dim=[CAM_DIM, Z_DIM])

                valid_counts = valid_mask.unflatten(0, (B, N)).sum(dim=[CAM_DIM, Z_DIM])
    ...
    def _get_valid_indices(self, coords_3d, fov_masks, feature_u, feature_v, image_d):
        B, V, Z, Y, X, _ = coords_3d.shape
        voxel_indices = torch.arange(
            B * X * Y * 1, device=coords_3d.device, dtype=self.index_dtype
        )
        voxel_indices = voxel_indices.view(B, 1, X, Y, 1)
        voxel_indices = voxel_indices.permute(0, 1, 4, 3, 2).contiguous().expand(B, V, Z, Y, X)
    ...
    def _calculate_coords_3d(
        ...
        coords_list_reversed = [coords_list[2], coords_list[1], coords_list[0]]
        coords_3d = torch.meshgrid(*coords_list_reversed, indexing="ij")
